...
void GripperExampleController::assignMoveGoalOptionsCallbacks() {
  move_goal_options_.goal_response_callback =
      [this](const std::shared_ptr<rclcpp_action::ClientGoalHandle<franka_msgs::action::Move>>& goal_handle) {
        if (!goal_handle) {
          RCLCPP_ERROR(get_node()->get_logger(), RED "Move Goal (open) NOT accepted." RESET);
        } else {
          RCLCPP_INFO(get_node()->get_logger(), "Move Goal accepted.");
        }
      };
  ...
  move_goal_options_.result_callback =
      [this](const rclcpp_action::ClientGoalHandle<franka_msgs::action::Move>::WrappedResult& result) {
        if (rclcpp_action::ResultCode::SUCCEEDED == result.code) {
          RCLCPP_INFO(get_node()->get_logger(), "Move Goal (open) SUCCESS, now closing gripper.");
          graspGripper();  // 열기 완료 후 닫기 실행
        } else {
          RCLCPP_ERROR(get_node()->get_logger(), RED "Move Goal (open) FAIL" RESET);
          // rclcpp::shutdown();  // 실패해도 종료
          std::exit(0); 
        }
      };
}
...
void GripperExampleController::assignGraspGoalOptionsCallbacks() {
  grasp_goal_options_.goal_response_callback =
      [this](const std::shared_ptr<rclcpp_action::ClientGoalHandle<franka_msgs::action::Grasp>>& goal_handle) {
        if (!goal_handle) {
          RCLCPP_ERROR(get_node()->get_logger(), RED "Grasp Goal NOT accepted." RESET);
        } else {
          RCLCPP_INFO(get_node()->get_logger(), "Grasp Goal accepted.");
  ...
  grasp_goal_options_.result_callback =
      [this](const rclcpp_action::ClientGoalHandle<franka_msgs::action::Grasp>::WrappedResult& result) {
        if (rclcpp_action::ResultCode::SUCCEEDED == result.code) {
          RCLCPP_INFO(get_node()->get_logger(), "Grasp (close) SUCCESS. Node will shutdown.");
        } else {
          RCLCPP_ERROR(get_node()->get_logger(), RED "Grasp (close) FAIL" RESET);
        }
        // rclcpp::shutdown();
        std::exit(0);  // 닫기 완료 후 노드 종료
      };
}

bool GripperExampleController::openGripper() {
  RCLCPP_INFO(get_node()->get_logger(), "Opening the gripper - submitting Move Goal");

  franka_msgs::action::Move::Goal move_goal;
  move_goal.width = 0.08;
  move_goal.speed = 0.2;

  auto move_goal_handle = gripper_move_action_client_->async_send_goal(move_goal, move_goal_options_);
  return move_goal_handle.valid();
}

void GripperExampleController::graspGripper() {
  RCLCPP_INFO(get_node()->get_logger(), "Closing the gripper - submitting Grasp Goal");
  ...
  auto grasp_goal_handle = gripper_grasp_action_client_->async_send_goal(grasp_goal, grasp_goal_options_);
  (void)grasp_goal_handle;
}
